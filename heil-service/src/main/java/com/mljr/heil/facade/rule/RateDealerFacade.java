 ...
 */
//@Component
public class RateDealerFacade implements BaseFacade<DealerVo,BaseDealerRes,BaseDealerRes,DealerQueryForm> {
    private final Logger LOGGER = LoggerFactory.getLogger(this.getClass());
    private final String LOG_TITLE = "利率规则门店管理";
    ...
    public Result<PageVO<DealerVo>> loadRecords(PageForm<DealerQueryForm> form) {
        ...
        int count = 0;
        try {
            list = this.rateDealerService.queryByPage(form);
            count = this.rateDealerService.queryCount(form);
    ...
    public Result<DealerVo> queryRecord(BaseDealerRes primaryKey) {
        ...
        try {
            record = this.rateDealerService.queryRecord(primaryKey);
            if (record == null) {
        ...
        LOGGER.info("{} - queryRecord 结果:{}",LOG_TITLE,JSON.toJSON(vo));
        return Result.suc(vo);
    }

    @Override
    public Result<String> saveRecord(BaseDealerRes record) {
        boolean isNotExists = !Optional.ofNullable(rateDealerService.queryRecord(record)).isPresent();
        return commonComponent.saveRecord(record,authModelEnum,LOG_TITLE,rateDealerService,entity -> {
    ...
    public Result<String> deleteRecord(BaseDealerRes primaryKey) {
        return commonComponent.deleteRecord(primaryKey,LOG_TITLE,rateDealerService,(PK) ->
